function AQ_SECAPI_ESCAPE(Url, map) {
	...
	for (var m = 0; m < Url.length; m++)
		if (Url.charAt(m) == "&") {
			...
			if (matchFlag == 0) tmpArr.push(Url.charAt(m))
		} else tmpArr.push(Url.charAt(m));
	return tmpArr.join("")
}
...
function AQ_SECAPI_CheckXss() {
	...
	for (var i = 0; i < escapeChars.length; i++) {
		...
		for (var j = 0; j < 7 - dec.toString().length; j++) dec7 = "0" + dec7;
		map["&#" + dec + ";"] = cha;
		map["&#" + dec7] = cha;
		map["&#x" + hex] = cha
	}
	...
	Refer = decodeURIComponent(AQ_SECAPI_ESCAPE(Refer,
		map));
...
(function(_wnd, _doc, _ln) {
	function checkNonTxDomain(level, bid, pagetype) {
		...
		if (pagetype == 1) try {
			...
			try {
				...
				parentCheckFlag = parent != _wnd ? generateNonTxDomainFromDom(parent.document,
					"datapp", checkInfo) : false
			} catch (ign) {}
			try {
				if (parent != _wnd) generateZyjIframed(checkInfo)
			} catch (ign) {}
		}
	}
	...
	function packZyjUrlData(data, dataMark, checkInfo) {
		...
		img.src = cgi + "?id=" + checkInfo.bid + "&d=" +
			dataMark + "=v" + version + "|" + data.join("|");
		return true
	}
	...
	function generateNonTxDomainFromDom(dom, parentMark, checkInfo) {
		...
		if (hacks.length <= 0) return false;
	...
	function extractNonTxScriptWorm(dom) {
		var scripts =
			...
			scriptData = [],
			tempScript, urlList, url, nonTxList, mapFunc, resultList;
		for (var i = 0; i < scripts.length; i++) {
			tempScript = scripts[i];
			if (url = tempScript.src) scriptData.push(url)
		}
	...
	function extractZyjUrlFromHtml(html) {
		var regUrl = /\bhttps?:\/\/[^\"\'\s]+/ig,
			urlList = [];
		while (url = regUrl.exec(html)) urlList.push(url);
		return urlList
	}

	function grepZyjList(testList,
		testFunction) {
		var grepList = [];
		for (var idx = 0; idx < testList.length; ++idx) {
			var temp = testList[idx];
			if (testFunction(temp)) grepList.push(temp)
		}
		return grepList
	}
	...
	function isAntiTxDomain(sUrl) {
		...
		if (regErrDom.test(sDomain) || (regTxCom.test(sDomain) || (regTxCn.test(sDomain) ||
				(regTxNet.test(sDomain) || regTxOther.test(sDomain))))) return false;
		return true
	}
	...
	function extractNonTxForm(dom) {
		...
		return extractNonTxTagData(dom, tagName, rawFunc, isAntiTxDomain,
			mapFunc)
	}
	...
	function distinctZyjArray(list) {
		...
		for (var i = 1; i < sortList.length; i += 1)
			if (sortList[i] != sortList[i - 1]) derivedArray.push(sortList[i]);
		return derivedArray
	}
...
(function() {
	...
	if (window.ActiveXObject || window.msIsStaticHTML) {
		...
		ua.isBeta = navigator.appMinorVersion &&
			navigator.appMinorVersion.toLowerCase().indexOf("beta") > -1;
		if (ua.ie < 7) try {
	...
	} else if (document.getBoxObjectFor || typeof window.mozInnerScreenX !=
		"undefined") {
		...
		for (; i < len; i++)
			if ((opts = arguments[i]) != null)
				for (var name in opts) {
					...
					if (deep && (copy &&
							(typeof copy === "object" && !copy.nodeType))) {
						if (src) clone = src;
	...
	each: function(obj, callback) {
		...
		if (isObj)
			for (var name in obj) {
				...
				for (value = obj[0]; i <
					length && false !== callback.call(value, value, i, obj); value = obj[++i]);
		return obj
	},
...
MUSIC.console = function(expr) {
	if (window.console)
		if (console.assert) console.assert.apply(null, arguments);
...
MUSIC.string = {
	...
	rtrim: function(str) {
		return MUSIC.string.commonReplace(str +
			"", MUSIC.string.RegExps.rtrim, "")
	},
	nl2br: function(str) {
	...
	replaceUrl: function(str) {
		return str.replace(/%/g,
			"%25").replace(/=/g, "%3D").replace(/&/g, "%26").replace(/\?/g, "%3F").replace(/#/g, "%23")
	},
	...
	GbkUrlDecode: function(str, callback) {
		...
		if (!!navigator && ("systemLanguage" in navigator && navigator.systemLanguage !=
				"zh-cn")) jsLoader.load(MUSIC.config.toolPath + "js/gbk_decode.js");
		else {
			var opts = {
				type: "text/vbscript"
			};
	...
	format: function(str) {
		...
		MUSIC.string.RegExps.format.lastIndex =
			0;
	...
	checkKoreaChar: function(str) {
		for (i = 0; i < str.length; i++)
			if (str.charCodeAt(i) > 12592 && str.charCodeAt(i) < 12687 || str.charCodeAt(i) >= 44032 && str.charCodeAt(i) <= 55203) return true;
		return false
	},
	escapeKoreaChar: function(str) {
		var dest = [];
		for (var i = 0; i < str.length; i++)
			if (str.charCodeAt(i) > 12592 && str.charCodeAt(i) < 12687 || str.charCodeAt(i) >= 44032 && str.charCodeAt(i) <= 55203) dest.push("&#" + str.charCodeAt(i) +
				";");
...
MUSIC.event = {
	...
	addEvent: function(obj, eventType, fn, argArray) {
		...
		if (obj.addEventListener) {
			...
			else if (eventType === "mouseleave") obj.addEventListener("mouseout",
				withinElement(cfn), false);
	...
	purgeEvent: function(obj, type) {
		...
		if (obj.eventsListUID && ((l = MUSIC.event._eventListDictionary[obj.eventsListUID]) && l[type]))
			for (var k in l[type])
				if (obj.removeEventListener) obj.removeEventListener(type, l[type][k], false);
	...
	replaceAllEvent: function(container) {
		...
		for (var i = 0, len = _arr.length; i < len; i++) replaceElmEvent(_arr[i], i)
	},
	onDomReady: function(fn) {
		...
		if (document.addEventListener)
			if (MUSIC.userAgent.safari < 4) var interval = setInterval(function() {
			...
			document.write('<script onreadystatechange="if(this.readyState==\'complete\'){this.parentNode.removeChild(this);MUSIC.event.onDomReady._fn();}" defer="defer" src="' +
				src + '">\x3c/script>')
		}
	}
};
...
MUSIC.lang = {
	...
	objectClone: function(obj, preventName) {
		if (typeof obj == "object") {
			...
			for (var i in obj)
				if (i != preventName) res[i] = MUSIC.lang.objectClone(obj[i], preventName);
			return res
	...
	propertieCopy: function(s, b, propertiSet) {
		if (typeof propertiSet == "undefined")
			for (var p in b) s[p] = b[p];
		else
			for (var p in propertiSet) s[p] = b[p];
		return s
	},
	chain: function(u, v) {
		var calls = [];
		...
		return function() {
			for (var ii = 0, len = calls.length; ii < len; ii++)
				if (calls[ii] && calls[ii].apply(null, arguments) === false) return false;
			return true
		}
	},
...
if (typeof Array.prototype.map != "function") Array.prototype.map = function(fn, context) {
	var arr = [];
	if (typeof fn === "function")
		for (var k = 0, length = this.length; k < length; k++) arr.push(fn.call(context, this[k], k, this));
	return arr
};
MUSIC.dom = {
	getById: function(id) {
	...
	getByName: function(name, tagName) {
		...
		for (var i = 0; i < e.length; ++i)
			if (!!e[i].getAttribute("name") && e[i].getAttribute("name").toLowerCase() == name.toLowerCase()) arr.push(e[i]);
		return arr
	},
	get: function(e) {
	...
	searchElementByClassName: function(elem, className) {
		...
		return MUSIC.dom.searchChain(elem, "parentNode", function(el) {
			return MUSIC.css.hasClassName(el,
				className)
		})
	},
	...
	createElementIn: function(tagName, elem, insertFirst, attrs) {
		...
		if (attrs)
			for (k in attrs)
				if (k == "class") _e.className = attrs[k];
				else if (k == "style") _e.style.cssText = attrs[k];
		else _e[k] = attrs[k];
		insertFirst ? elem.insertBefore(_e, elem.firstChild) :
			elem.appendChild(_e);
		return _e
	},
	...
	setStyle: function(el, properties, value) {
		if (!(el = MUSIC.dom.get(el)) || el.nodeType !=
			1) return false;
		var tmp, bRtn = true,
	...
	getPosition: function(elem) {
		...
		if (box = MUSIC.dom.getRect(elem)) {
			if (s = MUSIC.dom.getScrollLeft(doc =
					elem.ownerDocument)) box.left += s, box.right += s;
			if (s = MUSIC.dom.getScrollTop(doc)) box.top += s, box.bottom += s;
			return box
		}
	},
	...
	setXY: function(elem, x, y) {
		var _ml = parseInt(MUSIC.dom.getStyle(elem,
				"marginLeft")) || 0,
	...
	setSize: function(el, w, h) {
		...
		MUSIC.dom.setStyle(el, "height", (m =
			_r.exec(h)) ? m[1] ? h : parseInt(h, 10) + "px" : "auto")
	},
		...
		if (node.getElementsByTagNameNS) return node.getElementsByTagName(ns + ":" + tgn);
			...
			if (n.length > 0) {
				...
				for (var i = 0, len = l.length; i < len; ++i)
					if (l[i].scopeName == ns) res.push(l[i])
			}
		}
		return res
	},
	...
	insertAdjacent: function(elem, where, html, isText) {
		...
		if (MUSIC.lang.isElement(elem) && (pos[where] && (MUSIC.lang.isString(html) || MUSIC.lang.isElement(html)))) {
			if (elem.insertAdjacentHTML) elem["insertAdjacent" +
				(typeof html == "object" ? "Element" : isText ? "Text" : "HTML")](pos[where], html);
			else {
...
MUSIC.util = {
	...
	commonDictionarySplit: function(s,
		esp, vq, eq) {
		var res = {};
	...
	getUrlParams: function() {
		...
		MUSIC.object.extend(res,
			this.splitHttpParamString(uri.hash));
...
MUSIC.css = {
	...
	insertCSSLink: function(url, opts, callback) {
		...
		if (ua.ie || ua.opera) {
			MUSIC.event.addEvent(cssLink, ua.ie && ua.ie < 9 ? "readystatechange" : "load", function() {
				if (!cssLink || ua.ie && (ua.ie < 9 && !(cssLink.readyState == "loaded" || cssLink.readyState ==
						"complete"))) return;
	...
	addClassName: function(elem, names) {
		return MUSIC.css.updateClassName(elem,
			null, names)
	},
		...
		if (elems && typeof elems == "object") {
			...
			for (var elem, i = 0, l = elems.length; i < l; ++i)
				if ((elem = elems[i]) && elem.nodeType == 1)
					if (MUSIC.css.hasClassName(elem, a)) MUSIC.css.updateClassName(elem, a, b);
					else if (!_isRep && MUSIC.css.hasClassName(elem, b)) MUSIC.css.updateClassName(elem,
				b, a)
		}
	},
	...
	function stack(e, a) {
		function genTrace(ee, aa) {
			if (ee.stack) return ee.stack;
				...
				var r = [];
				for (var i = 0, len = eas.length; i < len; i++) r.push(typeof eas[i] == "undefined" ? "<u>" : eas[i] === null ? "<n>" : eas[i]);
		...
		switch (typeof value) {
			...
			case "object":
				...
				if (Object.prototype.toString.apply(value) === "[object Array]") {
					length = value.length;
					for (i =
						0; i < length; i += 1) partial[i] = str(i, value) || "null";
...
MUSIC.widget.trackServ = {
	...
	showSingleSong: function(obj, subtype) {
		...
		if (!!_music.songmid) setTimeout(function() {
			window.location.href = "http://i.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songmid=" + _music.songmid + "&tpl=yqq_song_detail"
		}, 0);
		else if (!!_music.dissID) setTimeout(function() {
			window.location.href = "http://i.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songid=" +
				_music.songid + "&tpl=yqq_song_detail&songtype=11&disstid=" + _music.dissID + "&play=0="
		}, 0)
	},
	play: function(obj, subtype) {
		...
		if (!!_music.songurl && (_music.songtype % 10 != 5 && (!/(\.m4a)|(\.mp3)/i.test(_music.songurl) && !/share\.weiyun\.qq\.com/i.test(_music.songurl)))) {
			g_popup.show(1,
				"\u6b4c\u66f2\u64ad\u653e\u5931\u8d25\uff01", "\u5f88\u62b1\u6b49\uff0cQQ\u97f3\u4e50\u6682\u4e0d\u63d0\u4f9b\u6b64\u683c\u5f0f\u7684\u6b4c\u66f2\u670d\u52a1\u3002", "", 5E3, 360);
			return
		}
		if (!subtype) subtype = obj.getAttribute("stat");
	...
	like: function(obj, subtype) {
		...
		if (obj.className == "btn_like") {
			if (!_music.song_type && !_music.songmid || _music.song_type && _music.song_type != 13) top.g_mymusicAlbumMod.addNetSongs2Dir({
				uin: uin,
				songtitle: _music.songname,
				singer: _music.singer[0].name,
				url: _music.songurl,
				dirid: 201
			}, function(d) {
				if (d &&
					(d.diy_id && d.diy_id.length > 0)) obj.setAttribute("songid", d.diy_id[0]);
				obj.className = "btn_liked";
				obj.title = "\u53d6\u6d88\u559c\u6b22";
				try {
					top.g_topPlayer.updateSongLike(d.diy_id[0], 1)
				} catch (e) {}
			}, true);
			else top.g_mymusicAlbumMod.Song.like(_music.songid, 3, function() {
				obj.className = "btn_liked";
				obj.title = "\u53d6\u6d88\u559c\u6b22";
				try {
			...
			if (!!obj.getAttribute("songid")) {
				_music.songid =
					obj.getAttribute("songid");
				_music.song_type = 11
			}
			top.g_mymusicAlbumMod.Song.unlike(_music.songid, !_music.songmid ? 11 : 3, function() {
	...
	fav: function(obj, dirinfo, subtype) {
		...
		if (!_music.songmid) top.g_mymusicAlbumMod.addNetSongs2Dir({
			uin: uin,
			songtitle: _music.songname,
			singer: _music.singer[0].name,
			url: _music.songurl,
		...
		else top.g_mymusicAlbumMod.addSongs2Dir({
			uin: uin,
	...
	initSongList: function(elem,
		...
		MUSIC.object.each(_list, function(_elem) {
			...
			for (var i = 0; i < _len; i++) _es[i].onclick = function(e) {
				return function() {
					g_trackServ.play(e);
					if (!!stat) pgvClickStat(stat + ".play")
				}
			}(_es[i]);
			_es = _es = MUSIC.dom.getElementsByClassName("btn_add",
				"a", _elem);
			_len = _es.length;
			for (var i = 0; i < _len; i++) _es[i].onclick = function(e) {
				return function() {
					g_trackServ.add(e);
					if (!!stat) pgvClickStat(stat + ".add")
				}
			}(_es[i]);
			...
			for (var i = 0; i < _len; i++) _es[i].onclick = function(e) {
				return function() {
					...
					if (!!stat) pgvClickStat(stat + ".share")
				}
			}(_es[i]);
				...
				for (var i = 0; i < _len; i++) {
					...
					_e.onclick = function(e) {
						return function() {
							pageHotClick();
							g_trackServ.like(e);
							if (!!stat) pgvClickStat(stat + ".like")
						}
					}(_e);
			...
			g_popup.init({
				...
				showCall: function(obj) {
					var obj = MUSIC.dom.searchChain(obj, null, function(elem) {
						if (elem.tagName &&
							elem.tagName.toUpperCase() == tag.toUpperCase()) return true;
						else return false
					});
	...
	getSongButtonHtml: function(objMusic, stattype, needTry) {
		...
		if (objMusic.action.fav) {
			obj.btn_like = '<a href="javascript:;" mid="' + (!!objMusic.songmid ? objMusic.songmid : objMusic.song_mid) + '" title="\u559c\u6b22" class="btn_like" stat="' + stattype + '">\u559c\u6b22</a>';
	...
	watchMv: function(vid, subtype) {
		window.open("http://i.y.qq.com/v8/fcg-bin/fcg_v8_mvout4web.fcg?cmd=1&format=html&tpl=mvplay&vid=" + vid.trim());
	...
	getSongsCount: function(container, tagName, flag, index) {
		...
		var j = new MUSIC.JSONGetter(url,
			"songsCount", null, "utf-8", false);
		j.onSuccess = function(data) {
			if (!!data && data.code == 0) {
				var max_listen = 0;
				...
				for (var i = 0; i < data.songlist.length; i++) {
					...
					var Count = isNaN(data.songlist[i].count) ? "\u6700\u65b0\u4e0a\u67b6" :
						data.songlist[i].count;
...
MUSIC.widget.user = {
	...
	openLoginPage: function(target) {
		...
		try {
			...
			_url = "http://y.qq.com/y/static/singer/index/all_hot_1.html"
		}
	...
	openLogin: function(url, target, logincallback, closecallback) {
		...
		if (isIphoneOs || isAndroid) {
			...
			if (isIphoneOs) loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?daid=384&pt_no_auth=1&style=8&appid=1006102&s_url=" + encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
			else loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?daid=384&pt_no_auth=1&style=9&appid=1006102&s_url=" +
				encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
	...
	openVip: function(aid, cm, defaultmonth) {
		if (typeof pgvSendClick !=
			"undefined")
			if ("music.y.sfw.1" == aid) pgvSendClick({
				hottag: "music.pc.20165.btnclick"
			});
			else pgvSendClick({
				hottag: "music.pc.20166.btnclick"
			});
		aid = aid || "";
		cm = cm || "";
	...
	openPayMusic: function(aid) {
		g_user.getVipInfo(function(userdata) {
			var type = 0;
			if (parseInt(userdata.twelve, 10) == 1) type = 1;
			aid = aid || "";
			var _sd = new MUSIC.JsLoader;
			_sd.onload = function() {
				var codes = [
...
var UserInfoCookie = MUSIC.CookieSet("vip", "nickname", "score", "place", "payway", "start", "end", "yearFlag", "yearstart", "yearend", "nowtime", "CurrentLevel", "UpgradeDayLeft", "NextLevelPoint", "UpgradePercentLeft", "EverydayPoint", "PrepaidByBankcard", "svip", "eight", "twelve", "yearFfb", "ffbLevel", "ffbScore");
...
MUSIC.widget.main = {
	init: function() {
		if (!/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=mvplay/.test(window.location.href))
			if (top == window && (/^http:\/\/(y|music)\.qq\.com\/y\/static\/(?!mv\/mv_play\.html)/.test(window.location.href) || (/^http:\/\/i\.y\.qq\.com\/s\.plcloud\/fcgi-bin\/fcg_yqq_song_detail_info_cp\.fcg/.test(window.location.href) || (/^http:\/\/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_play_single_song\.fcg/.test(window.location.href) || /^http:\/\/i\.y\.qq\.com\/v8\/([^#?$.]*)\.fcg/.test(window.location.href))))) {
				var simpleUrl =
					g_hashHistory.getSimpleUrl(window.location.href);
		...
		try {
			...
			if (top != window && top.window.location.href.indexOf("y.qq.com") >
				-1) {
				...
				var url = hparam["url"] || "";
				var frame_url = window.location.href.replace(/#.+/g, "");
				if (top_url != frame_url && (frame_url.indexOf("http://y.qq.com/y/static/mv/mv_play.html") < 0 || /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=mvplay/.test(frame_url))) g_hashHistory.locationHash();
		...
		setTimeout(function() {
			var js =
				new MUSIC.JsLoader;
			js.onload = function() {};
			js.load("http://ctc.qzonestyle.gtimg.cn/utp/dnsgetForMusic.js?max_age=2592000&ver=20140513", null, {
				charset: "utf-8"
			})
		}, 2E3)
	},
	...
	watchPage: function() {
		...
		var _id = 0;
		var _page = [/(y\.qq\.com)\/(index|\?|\#|$)/, /\/singer\/index\//, /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_singer_detail_cp\.fcg/, /\/album\/index\.html/, /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_album_detail_cp\.fcg/, /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_toplist_cp\.fcg\?([^#?$.]*)page=index/, /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_toplist_cp\.fcg\?([^#?$.]*)page=detail/, /\/toplist\/albumtop\//, /mv\/mv_toplist\.html/, /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg/,
			/\/mv\/lib\.html/, /mymusic_index\.html/, /mymusic_albumlist\.html/, /mymusic_mvlist\.html/, /mymusic_follow_listen\.html/, /mymusic\/album_detail\.html/, /\/mv\/mv_play\.html/, /s\.music\.qq\.com\/fcgi-bin\/yqq_search_v8_cp/, /fcg_play_single_song\.fcg/, /(v5|y\.qq\.com)\/(index|\?|\#|$)/
		];
		...
		if (/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=mvplay/.test(_url)) _id = 17;
		...
		function loadIPcgi(callback) {
			...
			j.onSuccess = function(data) {
				if (!!data && (data.ret == 0 || data.ret == 1)) {
					...
					var country = data.rec_app && data.rec_app.country ?
						data.rec_app.country : "";
...
(function() {
	...
	$.object.extend(SPD, {
		...
		markStart: function(name, time) {
			this.pointTime[name] = time || Date.now()
		},
		markEnd: function(name, time) {
			if (this.pointTime[name]) {
				this._timing[name] = (time || Date.now()) - this.pointTime[name];
				this.send();
		...
		send: function(times) {
			...
			if (!this.flag.length) return false;
			var t = new Image;
			...
			var params = {
				appid: "10013",
				speedparams: encodeURIComponent(this.flag.join("&") + "&" + buildArgs(_times)),
				platform: "unknown",
				app: "other"
			};
			t.src = "http://report.huatuo.qq.com/report.cgi?" + buildArgs(params);
		...
		init: function(opts) {
			...
			if (opts && opts.flag instanceof Array) {
				...
				if (opts.flag[3]) this.flag.push("flag4=" +
					opts.flag[3]);
				window.onload = MUSIC.lang.chain(window.onload, function() {
					SPD.mark(34);
					self._timing[35] = self._timing[31] + self._timing[32] + self._timing[33] + self._timing[34];
					setTimeout(function() {
						var timeList = {};
						if (window.performance && performance.timing) try {
							var navigationTime = performance.timing;
							var field = ["navigationStart", "unloadEventStart", "unloadEventEnd", "redirectStart", "redirectEnd", "fetchStart", "domainLookupStart", "domainLookupEnd", "connectStart", "connectEnd", "requestStart", "responseStart", "responseEnd",
							...
							self._timing[28] = navigationTime.connectEnd - navigationTime.connectStart;
							self._timing[29] = navigationTime.responseStart -
								navigationTime.requestStart;
							self._timing[30] = navigationTime.responseEnd - navigationTime.responseStart
						} catch (e) {}
						$.object.extend(self._timing, timeList);
						SPD.send()
					}, 100)
				})
			}
...
MUSIC.widget.watch = {
	...
	setTime: function() {
		this.timers = [];
		if (typeof g_watchCssBegin == "undefined" || (typeof g_watchPageBegin == "undefined" || (typeof g_watchJsBegin == "undefined" ||
				typeof g_watchRenderBegin == "undefined"))) return false;
		var _t, _userBegin = 0,
			_end = new Date - 0,
			_p = window.performance || (window.msPerformance || (window.webkitPerformance || null));
		if (_p && (_t = _p.timing)) _userBegin = _t.navigationStart || 0;
		if (_end - g_watchCssBegin > 1E4) return false;
		if (_userBegin) {
			this.timers[0] = _end - g_watchCssBegin + _t.requestStart - _userBegin;
			this.timers[1] = _t.requestStart - _userBegin
		}
		this.timers[2] = g_watchPageBegin - g_watchCssBegin;
		this.timers[3] = g_watchRenderBegin - g_watchJsBegin;
		this.timers[4] = _end -
			g_watchRenderBegin;
		this.timers[5] = _end - g_watchCssBegin;
		return true
	},
...
MUSIC.widget.statistics = {
	...
	getVirtualUrl: function(url) {
		...
		else if (/http:\/\/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_album_detail_cp\.fcg/.test(window.location.href)) return "/y/static/album/detail.html";
		...
		else if (/http:\/\/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_play_single_song\.fcg/.test(window.location.href)) return "/y/static/song/detail.html";
		else if (/http:\/\/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=tpl/.test(window.location.href)) return "/y/static/mv/index.html";
		else if (/http:\/\/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=mvplay/.test(window.location.href)) return "/y/static/mv/mv_play.html";
	...
	doPvg: function(url) {
		if (typeof pgvMain == "function") {
			...
			url = url.replace(new RegExp("pgv_ref"),
				"ADTAG");
	...
	getStatSource: function() {
		if (window == top) {
			if ((new RegExp("mv_play.html")).test(window.location.href) || /i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=mvplay/.test(window.location.href)) return 8;
			return 0
		}
		var reg_map = {
			"y/static/index.html": 1,
	...
	stat2: function(stat_arr) {
		...
		if (stat_arr_len < 6 || stat_arr_len >
			10) return;
		var uin = g_user.getUin();
		var base_url = "http://i.y.qq.com/portalcgi/fcgi-bin/statistic/cgi_musicportal_stat3.fcg?";
		var num = 10;
		...
		for (var i = 0; i < num; i++) {
			...
			for (var j =
					stat_arr_len; j < 10; j++) arg_list.push("a" + i + "-" + (j + 1) + "=0");
	...
	stat3: function(optcode, remark) {
		...
		g_user.getVipInfo(function(detail) {
			...
			var user_level = detail.vip ==
				1 ? g_user.countVipLevel(detail.score) : 0;
			if (optcode > 0) {
			...
			if (optcode > 0) {
				var url = base_url + "?click_time=" + (new Date).valueOf() + "&source=focus&oper_cmd=" + optcode + "&vip_type=" + user_type +
					"&user_level=" + user_level + "&remark=" + remark;
				try {
					(new Image).src = url
				} catch (e) {}
			}
...
MUSIC.widget.hashHistory = function() {
	...
	function getSimpleUrl(url) {
		...
		if (/\/y\/static\/down\/([^#?$.]*)/.test(url)) {
		...
		} else if (/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_v8_mvout4web\.fcg\?([^#?$.]*)tpl=tpl/.test(url)) hash =
			"type=mv&p=index";
		...
		else if (/\/y\/static\/singer\/index\/all_hot_1\.html\?singer_type=([^#$]*)/.test(url)) hash = "type=singer&p=" + encodeURIComponent(RegExp.$1);
			...
			else hash = "type=album&mid=" + RegExp.$3 +
				(RegExp.$4 ? "&play=" + RegExp.$4 : "");
			var adtag = getParameterNew(top.location.href, "ADTAG") || ""
			...
			else if (/page=index/.test(url)) hash =
			"type=toplist&p=top_index";
		else {
			if (/page=detail/.test(url)) {
				...
				} else hash = "type=toplist&p=" +
					encodeURIComponent([type, topid].join("_"))
			}
			...
			if (RegExp.$1.indexOf("mymusic_index") ||
				(RegExp.$1.indexOf("mymusic_albumlist") || (RegExp.$1.indexOf("mymusic_mvlist") || (RegExp.$1.indexOf("mymusic_follow_listen") || RegExp.$1.indexOf("mymusic_follow_listen"))))) hash = "type=mymusic&p=" + encodeURIComponent(RegExp.$1)
		...
		} else if (/\/y\/static\/album\/song_detail.html#songmid=([^#?$.]*)/.test(url) || (/i\.y\.qq\.com\/s\.plcloud\/fcgi-bin\/fcg_yqq_song_detail_info_cp\.fcg\?songmid=([^#?$.]*)/.test(url) || (/i\.y\.qq\.com\/s\.plcloud\/fcgi-bin\/fcg_yqq_song_detail_info_cp\.fcg\?songmid=([^#?$.]*)/.test(url) ||
				/i\.y\.qq\.com\/v8\/fcg-bin\/fcg_play_single_song\.fcg\?songmid=([^#?$.]*)/.test(url)))) {
			hash = "type=song&mid=" + RegExp.$1;
			var songtype = getParameterNew(url, "songtype") || 0,
				shareuin = getParameterNew(url, "shareuin") || 0,
				disstid = getParameterNew(url, "disstid") || 0;
			hash += (!!songtype ? "&songtype=" + songtype : "") + (!!shareuin ? "&shareuin=" + shareuin : "") + (!!disstid ? "&disstid=" + disstid : "")
		} else if (/\/soso.music.qq.com\/fcgi-bin\/multiple_music_search.fcg([^#$]*)/.test(url) || /fcgi-bin\/yqq_search_v8_cp([^#$]*)/.test(url)) hash =
			"type=soso&p=" + encodeURIComponent(RegExp.$1);
		...
		else if (/\/y\/static\/taoge\/taoge_list\.html/.test(url)) hash =
			"type=taogelist";
			...
			if (/^http:\/\//.test(msg.url)) {
				...
				if (cmd && cmd[1]) switch (cmd[1]) {
					...
					case "buysupergreen":
						...
						break;
					case "buy8yuan":
						g_user.buyVip("buy8yuan",
							msg.aid + "$songid" + song.songid);
						break;
					case "buy12yuan":
...
/*  |xGv00|f5a46f01a184d856fbd7c138b2037b25 */